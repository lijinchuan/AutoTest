...
namespace AutoTest.UI.UC
{
    ...
    public class DocPage : WebTab
    {
        ...
        public void InitDoc(TestPage testPage,TestCase testCase, string title = "文档")
        {
            if (testCase == null)
            {
                return;
            ...
            sbdoc.AppendLine();
            sbdoc.AppendLine(testCase.Desc);
            sbdoc.AppendLine();
            sbdoc.AppendLine("**请求URL：** ");
            sbdoc.AppendLine($"- ` {testPage.Url} `");
            sbdoc.AppendLine();
            sbdoc.AppendLine("**请求方式：**");
            sbdoc.AppendLine($"- {testCase.WebMethod.ToString()}");
            sbdoc.AppendLine();
            sbdoc.AppendLine("**请求参数说明：** ");
            sbdoc.AppendLine();
            sbdoc.AppendLine("|参数名|必选|类型|说明|");
            sbdoc.AppendLine("|:----    |:---|:----- |-----   |");
            var aPIParams = AutoTest.Data.DataStoreSwitcher.Current.Find<TestCaseParam>(nameof(TestCaseParam), nameof(TestCaseParam.TestCaseId), new object[] { testCase.Id }).ToList();
            ...
            var examplelist = AutoTest.Data.DataStoreSwitcher.Current.Find<TestCaseDocExample>(nameof(TestCaseDocExample), nameof(TestCaseDocExample.TestCaseId), new object[] { testCase.Id });
            sbdoc.AppendLine("**示例：** ");
            sbdoc.AppendLine();
